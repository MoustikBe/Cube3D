...
void cast_ray(t_game *game, t_ray *ray)
{
    ...
    if (ray->side == 0)
        ray->perpWallDist = (ray->mapX - ray->posX + (1 - ray->stepX) / 2.0f) / ray->rayDirX2;
    else
        ray->perpWallDist = (ray->mapY - ray->posY + (1 - ray->stepY) / 2.0f) / ray->rayDirY2;
	return ;
}

void ray_calcul(t_ray *ray, t_game *game)
{
	ray->ray_angle = game->pa - (ray->FOV / 2.0f) + ((float)ray->x / (float)ray->screenWidth) * ray->FOV;
	cast_ray(game, ray);
...
void drawing_wall_x(t_ray *ray, t_game *game, t_img *texture)
{
	...
	while (ray->y < ray->drawEnd)
	{
		...
		ray->color = texture->data[ray->textureY * texture->width + ray->textureX];
